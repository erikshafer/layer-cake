...
Whole-twin line counts re-run 2026-09-05 after the before twin's consumer shutdown fix (same method; see `docs/build-log.md`): before twin 2,142 raw / 1,776 non-blank (was 2,061 / 1,705; +81 / +71). Two slice 004 files edited, none created: `src/before/LayerCake.WebApi/Messaging/NotifyBakerConsumer.cs` (idempotent `StopAsync`, bounded graceful shutdown, in-flight gate) and `src/before/LayerCake.Infrastructure/Messaging/RabbitMqConnection.cs` (close before dispose). After twin unchanged at 703 / 581.
