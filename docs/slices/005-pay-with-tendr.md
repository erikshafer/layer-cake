...
Optional, fenced, and not part of the contract: Tendr also maps Wolverine's HTTP transport endpoints, and `AuthorizeCardHandler` answers `AuthorizeCard` sent as a message with `InvokeAsync<CardAuthorization>`, using the envelope id as the idempotency key and the same rules as the REST endpoint. It is the pocket answer to "and if the other side were mine?"; neither twin uses it.
...
- `Orders/Order.cs`: `PaymentStatus` and `PaymentAuthorizationId` on the entity, mapped in the same file. `PaymentStatus` carries a database default (`atPickup`): without it, the startup schema migration drops and recreates a populated `after.orders` instead of adding the column (see the build log, 2026-09-14).
